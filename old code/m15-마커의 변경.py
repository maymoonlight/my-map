...
m = folium.Map(location=[df_map['위도'].mean(), df_map['경도'].mean()], zoom_start=11, tiles='OpenStreetMap')
folium.TileLayer('OpenStreetMap', name='미배정/수행요원별').add_to(m) # 내가 원하는 이름(name)을 붙여서 배경지도를 추가합니다.
...
for _, row in df_map.iterrows():
    ...
    quick_pass = int(row.get('퀵패스', 0))      # 1이면 퀵패스 대상
    monitoring = str(row.get('모니터링', ''))   # '대상'이면 색상 변경
    ...
    # [C] 방문 전 (0회차) - 현장 파악 모드
        ...
        m_opacity = 1.0 if quick_pass == 1 else 0.8

        # [변수 2] 업종 및 모니터링 대상 여부에 따른 핀 색상(m_color) 결정
        # 1. 특화분야 (Purple 계열)
        if '특화' in spec_field:
            m_color = 'purple' if monitoring == '대상' else 'darkpurple'
            i_name = BATTERY_ICONS.get(v_count, 'battery-full')  # 5단계
            
        # 2. 일반 제조 (Blue 계열)
        elif '제조' in spec_field:
            m_color = 'blue' if monitoring == '대상' else 'darkblue'
            i_name = BATTERY_ICONS.get(v_count, 'battery-full')  # 5단계
            
        # 3. 일반 기타 (Green 계열)
        else:
            m_color = 'green' if monitoring == '대상' else 'darkgreen'
