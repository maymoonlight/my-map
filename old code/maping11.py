...
# [수정] 수행요원별 '개별 클러스터' 생성 (전체 합산 바구니 삭제)
agent_clusters = {}
agent_col = '수행요원' if '수행요원' in valid_df.columns else '담당자'
for agent in valid_df[agent_col].unique():
    # 각 요원별로 독립된 클러스터를 만들어 토글이 100% 독립적으로 작동하게 합니다.
    cluster = plugins.MarkerCluster(name=f"수행요원: {agent}").add_to(m)
...
for _, row in valid_df.iterrows():
    ...
    # [핵심] 수행요원별 개별 클러스터에만 추가 (전체 클러스터 삭제됨)
    folium.Marker(
        location=[lat, lon], 
        popup=folium.Popup(popup_html, max_width=300), 
        tooltip=row['사업장명_공사장명'], 
        icon=icon
    ).add_to(agent_clusters[row[agent_col]])

# 6. 범례 및 제어 도구 (collapsed=True로 닫힘 설정)
...
# [수정] 레이어 제어 메뉴를 처음에는 닫힌 상태로 표시 (collapsed=True)
